...
import { Download, UserPlus, Search, Trash2 } from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import toast from 'react-hot-toast'
import api from '../api'
import { PageHeader, Card, Spinner, Empty, ConfirmModal, Badge, Avatar } from '../components/UI'
...
export default function WaitlistPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.getWaitlist({ limit: PAGE, offset, search: search.trim() || undefined })
  ...
  const confirmDelete = async () => {
    if (!pending) return
    try {
      await api.deleteWaitlistEntry(pending.id)
  ...
  const exportCSV = () => {
    ...
    const rows = items.map(r =>
      [r.name, r.email, r.source ?? '', r.created_at ?? ''].map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')
    ).join('\n')
    const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href = url
    a.download = `uteriflow-waitlist-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
  ...
  return (
    <div>
      <PageHeader
        title="Waitlist"
        subtitle={`${total} ${total === 1 ? 'person' : 'people'} waiting for early access.`}
      />
      ...
      <Card style={{ padding:'0', overflow:'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding:'48px' }}>
            <Empty icon="🌸" message={search ? 'No matching entries.' : 'No waitlist signups yet.'} />
          </div>
        ) : (
                  ...
                  <tr key={e.id} style={{ borderBottom:'1px solid var(--gray-100)' }}>
                    <td style={td}>
                      <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                        <Avatar name={e.name} email={e.email} size={32} />
                        <span style={{ fontWeight:500, color:'var(--gray-900)' }}>{e.name || '—'}</span>
                      </div>
                    </td>
                    <td style={{ ...td, color:'var(--gray-700)' }}>{e.email}</td>
                    <td style={td}><Badge color="gray">{e.source || 'landing'}</Badge></td>
                    <td style={{ ...td, color:'var(--gray-500)' }}>
                      {e.created_at ? formatDistanceToNow(new Date(e.created_at), { addSuffix:true }) : '—'}
                    </td>
                    <td style={{ ...td, textAlign:'right' }}>
                      <button
                        onClick={() => setPending({ id: e.id, email: e.email })}
                        ...
                        onMouseEnter={ev => ev.currentTarget.style.background = 'var(--red-light)'}
      ...
      <ConfirmModal
        ...
        title="Remove from waitlist?"
        description={pending ? `${pending.email} will be removed from the waitlist. This cannot be undone.` : ''}
        icon="🗑️"
        confirmLabel="Remove"
      />
    </div>
