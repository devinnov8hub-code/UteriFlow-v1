  ...
  return (
    <div style={{
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        {!imgFailed ? (
          <img
            src="/admin/dazzle-working-dashboard-auth-flow.png"
...
export default function AuthPage() {
  ...
  async function handleLogin(e) {
    ...
    setLoading(true)
    try { await login(loginEmail, loginPass); nav('/admin/analytics') }
  ...
  async function handleVerifyAndCreate(e) {
    ...
    try {
      ...
      if (data.session) { saveSession(data.session.accessToken, data.session.refreshToken, data.session.expiresAt); toast.success('Account created!'); nav('/admin/analytics') }
